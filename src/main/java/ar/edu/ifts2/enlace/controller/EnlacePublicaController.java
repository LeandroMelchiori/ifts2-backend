...
public class EnlacePublicaController {
    private final EnlaceService service;
    public EnlacePublicaController(EnlaceService service) { this.service = service; }

    @GetMapping
    @Operation(summary = "Listar enlaces publicados", description = "Solo PUBLICADA. Orden editorial ascendente. Las URLs de archivos son null si storage esta deshabilitado.")
    public PageResponse<EnlacePublicaResponse> listar(
    ...
    @GetMapping("/{id}")
    @Operation(summary = "Consultar enlace publicado", description = "Borradores y archivados responden 404 incluso con JWT.")
    public EnlacePublicaResponse obtener(@PathVariable UUID id) { return service.obtenerPublicada(id); }
}
