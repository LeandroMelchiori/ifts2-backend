...
@Tag(name = "Carreras Administration")
...
public class CarreraAdminController {
    ...
    public CarreraAdminController(CarreraService service) { this.service = service; }

    @GetMapping
    @Operation(summary = "Listar carreras para administracion", description = "Todos los estados salvo filtro opcional; orden createdAt descendente e id ascendente.")
    @ApiResponse(responseCode = "200", description = "Pagina administrativa")
    public PageResponse<CarreraAdminResponse> listar(
    ...
    @GetMapping("/{id}")
    @Operation(summary = "Consultar carrera en cualquier estado")
    @ApiResponse(responseCode = "200", description = "Recurso encontrado")
    public CarreraAdminResponse obtener(@PathVariable UUID id) { return service.obtener(id); }

    @PostMapping
    @Operation(summary = "Crear carrera como borrador")
    @ApiResponse(responseCode = "201", description = "Borrador creado; Location identifica el recurso")
    public ResponseEntity<CarreraAdminResponse> crear(@Valid @RequestBody CarreraRequest request) {
        var response = service.crear(request);
        return ResponseEntity.created(URI.create("/api/admin/carreras/" + response.id())).body(response);
    }

    @PutMapping("/{id}")
    @Operation(summary = "Reemplazar contenido de carrera", description = "Conserva estado y archivos. Los cambios de contenido publicado son visibles inmediatamente. No admite estado, timestamps ni objectKey en el body.")
    @ApiResponse(responseCode = "200", description = "Contenido actualizado")
    public CarreraAdminResponse actualizar(@PathVariable UUID id, @Valid @RequestBody CarreraRequest request) {
        return service.actualizar(id, request);
    }

    @PutMapping("/{id}/estado")
    @Operation(summary = "Publicar, retirar o archivar carrera", description = "BORRADOR y ARCHIVADA ocultan sin borrar. Repetir estado es idempotente; republicar renueva publicadaAt.")
    @ApiResponse(responseCode = "200", description = "Estado actualizado")
    public CarreraAdminResponse cambiarEstado(@PathVariable UUID id, @Valid @RequestBody CambiarEstadoRequest request) {
        return service.cambiarEstado(id, request.estado());
    }

    @PutMapping(value = "/{id}/imagen", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    @Operation(summary = "Subir o reemplazar imagen", description = "JPEG, PNG o WEBP, limite de imagenes (5 MB por defecto). MIME y firma validados. Clave generada bajo carreras/. Reemplazo compensado con PostgreSQL; limpieza fallida requiere revision manual.")
    ...
    @Operation(summary = "Quitar imagen", description = "Idempotente si no hay archivo. No elimina el contenido. Limpieza posterior al commit.")
