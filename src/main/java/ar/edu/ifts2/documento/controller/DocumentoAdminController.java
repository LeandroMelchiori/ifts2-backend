...
@Tag(name = "Documentos Administration")
...
public class DocumentoAdminController {
    ...
    public DocumentoAdminController(DocumentoService service) { this.service = service; }

    @GetMapping
    @Operation(summary = "Listar documentos para administracion", description = "Todos los estados salvo filtro opcional; orden createdAt descendente e id ascendente.")
    @ApiResponse(responseCode = "200", description = "Pagina administrativa")
    public PageResponse<DocumentoAdminResponse> listar(
    ...
    @GetMapping("/{id}")
    @Operation(summary = "Consultar documento en cualquier estado")
    @ApiResponse(responseCode = "200", description = "Recurso encontrado")
    public DocumentoAdminResponse obtener(@PathVariable UUID id) { return service.obtener(id); }

    @PostMapping
    @Operation(summary = "Crear documento como borrador")
    @ApiResponse(responseCode = "201", description = "Borrador creado; Location identifica el recurso")
    public ResponseEntity<DocumentoAdminResponse> crear(@Valid @RequestBody DocumentoRequest request) {
        var response = service.crear(request);
        return ResponseEntity.created(URI.create("/api/admin/documentos/" + response.id())).body(response);
    }

    @PutMapping("/{id}")
    @Operation(summary = "Reemplazar contenido de documento", description = "Conserva estado y archivos. Los cambios de contenido publicado son visibles inmediatamente. No admite estado, timestamps ni objectKey en el body.")
    @ApiResponse(responseCode = "200", description = "Contenido actualizado")
    public DocumentoAdminResponse actualizar(@PathVariable UUID id, @Valid @RequestBody DocumentoRequest request) {
        return service.actualizar(id, request);
    }

    @PutMapping("/{id}/estado")
    @Operation(summary = "Publicar, retirar o archivar documento", description = "Requiere PDF para PUBLICADA. BORRADOR y ARCHIVADA ocultan sin borrar. Repetir estado es idempotente; republicar renueva publicadaAt.")
    @ApiResponse(responseCode = "200", description = "Estado actualizado")
    public DocumentoAdminResponse cambiarEstado(@PathVariable UUID id, @Valid @RequestBody CambiarEstadoRequest request) {
        return service.cambiarEstado(id, request.estado());
    }

    @PutMapping(value = "/{id}/archivo", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    @Operation(summary = "Subir o reemplazar archivo", description = "Solo PDF, limite de documentos (10 MB por defecto). MIME y firma validados. Clave generada bajo documentos/. Reemplazo compensado con PostgreSQL; limpieza fallida requiere revision manual.")
    ...
    @Operation(summary = "Quitar archivo", description = "Requiere retirar primero el documento publicado a BORRADOR. Idempotente si no hay archivo. No elimina el contenido. Limpieza posterior al commit.")
